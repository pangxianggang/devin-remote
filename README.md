...
| **dao-vsix** | `3.23.0` | `dao.dao-vsix` | 本源基座：rt-flow 切号视图 + Devin Cloud 全功能面板 + 本地 HTTP API（含多账号反向注入）。3.23.0 新增通用型 MCP 一键修复(自动识别 node 运行时/模块路径、修正命令并启用)+ 实测使用(真起进程拿工具数)。可单独安装。 | [Release](https://github.com/zhouyoukang1234-spec/devin-remote/releases/tag/dao-vsix-v3.23.0) · [⬇ VSIX](https://github.com/zhouyoukang1234-spec/devin-remote/releases/download/dao-vsix-v3.23.0/dao-vsix-3.23.0.vsix) |
