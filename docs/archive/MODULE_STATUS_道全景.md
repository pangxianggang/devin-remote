...
| **rt-flow-app**⑥Devin Cloud 手机版 | `addons/rt-flow-app/`（独立 APK：Java 薄壳 + assets/engine JS） | 0.7.0 | **S23 Ultra 真机实测通过**：1:1 桌面 RT Flow 面板移植 + per-account 展开 + 穿透动态配置 + 手机适配化简 | 取代了旧的 rt-flow-mobile（MV3 扩展）和 dao-bridge-android（Termux Agent） |
