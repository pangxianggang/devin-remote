...
候补层  ⏳ 未验证账号                         100
−∞      永禁  无密码 / skipAutoSwitch（无"过期"判死）
```

> **v4.8.5 · 去"过期"判死**：`planEnd` 过期**不再**排除账号 —— Free 试用号到期后 D/W 仍可满额、完全可用。能否切号只看真实额度（D/W 门槛 + 双零守门）与黑名单（真实登录失败）。临期加成仅对**未来到期**生效。面板同步：账号整体发亮，唯真不可登录（拉黑）才变灰，去除"已过期 / 陈年 / Nh前"等标签。
