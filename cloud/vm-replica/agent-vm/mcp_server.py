...
TOOLS = {
    ...
    'vm_flow_probe':   ('vm.flow_probe',    "ADVISORY temporal/optical-flow cue for CONTINUOUS canvas motion: perform a drag {x,y,x2,y2} while sampling sub-frames of {region} (held button), then report the dominant motion axis (Lucas-Kanade) -- axis>0 horizontal (e.g. rotate), <0 vertical (e.g. tilt). A single before/after frame is direction-blind; this leans correctly on average but is NOT a confident classifier on arbitrary surfaces (margin small, resolution-dependent) -- use as a hint, escalate to vision for certainty.",
